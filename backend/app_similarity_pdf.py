...
from app_similarity import search_similar
...
def extract_authors_and_date(page, title_line_top):
    ...
    authors_raw = re.sub(r'(,\s*)?[¹²³⁴⁵⁶⁷⁸⁹⁰\d*]+', '', authors_raw)
    authors_raw = re.sub(r'\S+@\S+', '', authors_raw)
    authors_raw = re.split(r'\b(Faculty|University|Department|Institute|School|Center|Khoa|Trường)\b', authors_raw)[0]
    authors_clean = re.sub(r'\s+', ' ', authors_raw).strip()
    if not authors_clean:
...
async def extract_images_from_pdf(pdf: UploadFile = File(...)):
    try:
        images_data = []
        ...
        for page_num in range(len(doc)):
            ...
            pdf_plumber_page = pdf_plumber_doc.pages[page_num]
            text = pdf_plumber_page.extract_text()

            # Chỉ thực hiện trích xuất metadata ở trang đầu tiên
            if text and page_num == 0:
                # Trích tiêu đề và vị trí dòng cuối tiêu đề
                title, title_line_top = extract_title_by_fontsize(pdf_plumber_page)

                # Trích tác giả và ngày duyệt
                authors, approved_date = extract_authors_and_date(pdf_plumber_page, title_line_top)

                # Trích DOI
                doi = extract_doi(text)

            # Phát hiện số trang thực
            words = pdf_plumber_page.extract_words()
            bottom_texts = [obj for obj in words if obj["top"] > (pdf_plumber_page.height * 0.85)]
            center_x = pdf_plumber_page.width / 2
            ...
            # Phân tích layout của trang để tìm hình ảnh và caption
            page_dict = pdf_plumber_page.to_dict()
        ...
        # return JSONResponse(content={'images': images_data}, status_code=200)
...
@router.post("/search-similar-list")
...
#     uvicorn.run("app_similarity_pdf:app", host="0.0.0.0", port=8002, reload=True)
